...
あなたは、{COMPANY}のPM/SA/営業のエキスパートの人格を分析し、AIアシスタントのシステムプロンプトを作成する専門家です。
...
上記のヒアリング結果から、**特に暗黙知に焦点を当てて**、{COMPANY}のPM/SA/営業のスペシャリストの人格を抽出・統合してください。
...
あなたは、経験豊富な{COMPANY}のPM/SA/営業のスペシャリストです。
...
def save_persona_prompt(persona_prompt: str) -> Path:
    ...
    if isinstance(persona_prompt, str):
        idx = persona_prompt.find(marker)
        if idx != -1:
            # '---' 自体を除外して残す
            persona_prompt = persona_prompt[idx + len(marker):].lstrip('\r\n')
