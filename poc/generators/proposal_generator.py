...
"""
    ...
    def _search_similar_proposals(
        ...
        6.  実施スケジュールには根拠となる工数を出力してください。
        7.  (Must)期待される効果の定量的効果では断定表現を避けて目安であることをしめすため、「目標とします」「目指します」といった語尾にしてください。
...
"""
        ...
        try:
            ...
            proposal = extract_content(response)
...
"""
    ...
    def generate(
        ...
        # RAG結果を保存（思考プロセス分析用）
        # 類似提案書とヒアリングシートの両方を含める
        self._last_rag_results = f"""# 類似提案書
{similar_cases_text}
